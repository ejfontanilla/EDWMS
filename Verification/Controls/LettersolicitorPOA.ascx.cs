...
public partial class Verification_Control_LettersolicitorPOA : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "LettersolicitorPOA";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataLettersolicitorPOAEnum.DateOfDocument.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfDocument.DateIdValue.Length == 0)
            DateOfDocument.DateIdValue = metadataDb.Insert(DocTypeMetaDataLettersolicitorPOAEnum.DateOfDocument.ToString(), DateOfDocument.DateValue, docId, false).ToString();
        else
