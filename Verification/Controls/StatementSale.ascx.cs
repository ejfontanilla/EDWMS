...
public partial class Verification_Control_StatementSale : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "StatementSale";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataStatementSaleEnum.DateOfStatement.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfStatement.DateIdValue.Length == 0)
            DateOfStatement.DateIdValue = metadataDb.Insert(DocTypeMetaDataStatementSaleEnum.DateOfStatement.ToString(), DateOfStatement.DateValue, docId, false).ToString();
        else
