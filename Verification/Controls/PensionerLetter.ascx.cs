...
public partial class Verification_Control_PensionerLetter : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "PensionerLetter";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataPensionerLetterEnum.DateOfDocument.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfDocument.DateIdValue.Length == 0)
            DateOfDocument.DateIdValue = metadataDb.Insert(DocTypeMetaDataPensionerLetterEnum.DateOfDocument.ToString(), DateOfDocument.DateValue, docId, false).ToString();
        else
