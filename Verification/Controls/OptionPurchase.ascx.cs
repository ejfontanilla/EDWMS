...
public partial class Verification_Controls_OptionPurchase : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "OptionPurchase";
    ...
    public void ClearData()
    {
        ...
        DateOfOption.DateValue = string.Empty;
        DateOfOption.DateIdValue = string.Empty;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataOptionPurchaseEnum.DateOfOption.ToString().ToLower()))
                {
                    DateOfOption.DateValue = Format.GetMetaDataValueInMetaDataDateFormat(metaDataRow.FieldValue);
                    DateOfOption.DateIdValue = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        bool validDate = true;
        lblDateRangeError.Visible = false;
        if (Validation.IsValidDateRange(DateOfOption.DateValue, DateTime.Now.ToString()) || !isValidate)
        {
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate, bool IsBlurOrIncomplete)
    {
        ...
        if (IsBlurOrIncomplete && string.IsNullOrEmpty(DateOfOption.DateValue))
            strDateNow = string.Empty;

        if (Validation.IsValidDateRange(DateOfOption.DateValue, strDateNow, IsBlurOrIncomplete) || !isValidate)
        {
            SaveDataDetails(docId, docRefId, referencePersonalTable);
        }
        else
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfOption.DateIdValue.Length == 0)
            DateOfOption.DateIdValue = metadataDb.Insert(DocTypeMetaDataOptionPurchaseEnum.DateOfOption.ToString(), DateOfOption.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfOption.DateIdValue), DateOfOption.DateValue);
    }
    #endregion
