...
public partial class Verification_Control_BankStatement : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "BankStatement";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataBankStatementEnum.StartDate.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataBankStatementEnum.EndDate.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (FromDate.DateIdValue.Length == 0)
            FromDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataBankStatementEnum.StartDate.ToString(), FromDate.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(FromDate.DateIdValue), FromDate.DateValue);

        if (ToDate.DateIdValue.Length == 0)
            ToDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataBankStatementEnum.EndDate.ToString(), ToDate.DateValue, docId, false).ToString();
        else
