...
public partial class Verification_Control_EmploymentPass : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "EmploymentPass";
    ...
    public void ClearData()
    {
        ...
        DateOfExpiry.DateValue = string.Empty;
        DateOfExpiry.DateIdValue = string.Empty;
        lblDateRangeError.Visible = false;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataPassportEnum.DateOfExpiry.ToString().ToLower()))
                {
                    DateOfExpiry.DateValue = Format.GetMetaDataValueInMetaDataDateFormat(metaDataRow.FieldValue);
                    DateOfExpiry.DateIdValue = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        bool validDate = true;
        lblDateRangeError.Visible = false;
        if (Validation.IsValidDateRangePayExp(DateTime.Now.ToString(), DateOfExpiry.DateValue) || !isValidate)
        {
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate, bool IsBlurOrIncomplete)
    {
        ...
        if (IsBlurOrIncomplete && string.IsNullOrEmpty(DateOfExpiry.DateValue))
            strDateNow = string.Empty;

        if (Validation.IsValidDateRangePayExp(strDateNow, DateOfExpiry.DateValue, IsBlurOrIncomplete) || !isValidate)
        {
            SaveDataDetails(docId, docRefId, referencePersonalTable);
        }
        else
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfExpiry.DateIdValue.Length == 0)
            DateOfExpiry.DateIdValue = metadataDb.Insert(DocTypeMetaDataPassportEnum.DateOfExpiry.ToString(), DateOfExpiry.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfExpiry.DateIdValue), DateOfExpiry.DateValue);
    }
    #endregion
