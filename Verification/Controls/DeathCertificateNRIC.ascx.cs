...
public partial class Verification_Control_DeathCertificateNRIC : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeathCertificateNRIC";
    ...
    public void ClearData()
    {
        ...
        NameNRIC.Text = IdentityNoNRIC.Text = DateOfDeath.DateValue = string.Empty;
        ...
        NameNRICId.Value = IdentityNoNRICId.Value = string.Empty;
        lblDateRangeError.Visible = false;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateNRICEnum.DateOfDeath.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateNRICEnum.Tag.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateNRICEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateNRICEnum.IdentityNoNRIC.ToString().ToLower()))
                {
                    IdentityNoNRIC.Text = metaDataRow.FieldValue;
                    IdentityNoNRICId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateNRICEnum.NameNRIC.ToString().ToLower()))
                {
                    NameNRIC.Text = metaDataRow.FieldValue;
                    NameNRICId.Value = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    //{
    //    DateOfDeath.EnableRequiredField = RequiredFieldValidatorNameNRIC.Enabled = isEnabled;
    //    RequiredFieldValidatorIdentityNoOfNRIC.Enabled = NricFinCustomValidator.Enabled = isEnabled;
    //}
    #endregion
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateOfDeath.DateIdValue.Length == 0)
            DateOfDeath.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeathCertificateNRICEnum.DateOfDeath.ToString(), DateOfDeath.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeath.DateIdValue), DateOfDeath.DateValue);

        if (TagRadioButtonListId.Value.Length == 0)
            TagRadioButtonListId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateNRICEnum.Tag.ToString(), TagRadioButtonList.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(TagRadioButtonListId.Value), TagRadioButtonList.SelectedValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateNRICEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
        ...
        if (isValidate)
            NameNRIC.Text = Validation.ReplaceNonLetterCharacters(NameNRIC.Text);

        if (IdentityNoNRICId.Value.Length == 0)
            IdentityNoNRICId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateNRICEnum.IdentityNoNRIC.ToString(), IdentityNoNRIC.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoNRICId.Value), IdentityNoNRIC.Text);

        if (NameNRICId.Value.Length == 0)
            NameNRICId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateNRICEnum.NameNRIC.ToString(), NameNRIC.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(NameNRICId.Value), NameNRIC.Text);
    }
    #endregion
