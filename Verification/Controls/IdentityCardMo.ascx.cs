...
public partial class Verification_Controls_IdentityCardMo : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "IdentityCardMo";
    ...
    public void ClearData()
    {
        ...
        NameOfMother.Text = IdentityNo.Text = string.Empty;
        IDType.ClearSelection();
        IDTypeId.Value = string.Empty;
        NameOfMotherId.Value = IdentityNoId.Value = string.Empty;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardMoEnum.DateOfIssue.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardMoEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardMoEnum.IdentityNo.ToString().ToLower()))
                {
                    IdentityNo.Text = metaDataRow.FieldValue;
                    IdentityNoId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardMoEnum.NameOfMother.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateIssue.DateIdValue.Length == 0)
            DateIssue.DateIdValue = metadataDb.Insert(DocTypeMetaDataIdentityCardMoEnum.DateOfIssue.ToString(), DateIssue.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateIssue.DateIdValue), DateIssue.DateValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardMoEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IDTypeId.Value), IDType.SelectedValue);

        if (IdentityNoId.Value.Length == 0)
            IdentityNoId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardMoEnum.IdentityNo.ToString(), IdentityNo.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoId.Value), IdentityNo.Text);

        if (NameOfMotherId.Value.Length == 0)
            NameOfMotherId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardMoEnum.NameOfMother.ToString(), NameOfMother.Text, docId, false).ToString();
        else
    ...
    {
        CommonMetadata.EnableRequiredField = RequiredFieldValidatorIdentityNo.Enabled = RequiredFieldValidatorNameOfMother.Enabled = NricFinCustomValidator.Enabled = isEnabled;
    }
}