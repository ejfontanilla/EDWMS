...
public partial class Verification_Controls_IdentityCardNRIC : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "IdentityCardNRIC";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardNRICEnum.DateOfIssue.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardNRICEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardNRICEnum.IdentityNo.ToString().ToLower()))
                {
                    IdentityNo.Text = metaDataRow.FieldValue;
                    IdentityNoId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIdentityCardNRICEnum.NameOfNRIC.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateIssue.DateIdValue.Length == 0)
            DateIssue.DateIdValue = metadataDb.Insert(DocTypeMetaDataIdentityCardNRICEnum.DateOfIssue.ToString(), DateIssue.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateIssue.DateIdValue), DateIssue.DateValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardNRICEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IDTypeId.Value), IDType.SelectedValue);

        if (IdentityNoId.Value.Length == 0)
            IdentityNoId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardNRICEnum.IdentityNo.ToString(), IdentityNo.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoId.Value), IdentityNo.Text);

        if (NameOfChildId.Value.Length == 0)
            NameOfChildId.Value = metadataDb.Insert(DocTypeMetaDataIdentityCardNRICEnum.NameOfNRIC.ToString(), NameOfChild.Text, docId, false).ToString();
        else
