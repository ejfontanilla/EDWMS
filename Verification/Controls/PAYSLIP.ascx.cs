...
public partial class Verification_Control_PAYSLIP : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "PAYSLIP";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            foreach (MetaData.MetaDataRow metaDataRow in metaDatas.Rows)
            {
                ...
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataPAYSLIPEnum.StartDate.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataPAYSLIPEnum.EndDate.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (FromDate.DateIdValue.Length == 0)
            FromDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataPAYSLIPEnum.StartDate.ToString(), FromDate.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(FromDate.DateIdValue), FromDate.DateValue);

        if (ToDate.DateIdValue.Length == 0)
            ToDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataPAYSLIPEnum.EndDate.ToString(), ToDate.DateValue, docId, false).ToString();
        else
