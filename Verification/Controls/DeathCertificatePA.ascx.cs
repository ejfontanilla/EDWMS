...
public partial class Verification_Controls_DeathCertificatePA : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeathCertificatePA";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificatePAEnum.DateOfDeath.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificatePAEnum.Tag.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificatePAEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificatePAEnum.IdentityNoOfParent.ToString().ToLower()))
                {
                    IdentityNoOfMother.Text = metaDataRow.FieldValue;
                    IdentityNoOfMotherId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificatePAEnum.NameOfParent.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateOfDeath.DateIdValue.Length == 0)
            DateOfDeath.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeathCertificatePAEnum.DateOfDeath.ToString(), DateOfDeath.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeath.DateIdValue), DateOfDeath.DateValue);

        if (TagRadioButtonListId.Value.Length == 0)
            TagRadioButtonListId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificatePAEnum.Tag.ToString(), TagRadioButtonList.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(TagRadioButtonListId.Value), TagRadioButtonList.SelectedValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificatePAEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IDTypeId.Value), IDType.SelectedValue);

        if (IdentityNoOfMotherId.Value.Length == 0)
            IdentityNoOfMotherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificatePAEnum.IdentityNoOfParent.ToString(), IdentityNoOfMother.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoOfMotherId.Value), IdentityNoOfMother.Text);

        if (NameOfMotherId.Value.Length == 0)
            NameOfMotherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificatePAEnum.NameOfParent.ToString(), NameOfMother.Text, docId, false).ToString();
        else
