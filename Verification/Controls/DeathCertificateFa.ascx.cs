...
public partial class Verification_Control_DeathCertificateFa : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeathCertificateFa";
    ...
    public void ClearData()
    {
        ...
        NameOfFather.Text = IdentityNoOfFather.Text = DateOfDeath.DateValue = string.Empty;
        ...
        NameOfFatherId.Value = NameOfFatherId.Value = string.Empty;
        lblDateRangeError.Visible = false;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateFaEnum.DateOfDeath.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateFaEnum.Tag.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateFaEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateFaEnum.IdentityNoOfFather.ToString().ToLower()))
                {
                    IdentityNoOfFather.Text = metaDataRow.FieldValue;
                    IdentityNoOfFatherId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateFaEnum.NameOfFather.ToString().ToLower()))
                {
    ...
    //{
    //    CommonMetadata.EnableRequiredField = DateOfDeath.EnableRequiredField = RequiredFieldValidatorIdentityNoOfFather.Enabled = isEnabled;
    //    RequiredFieldValidatorNameOfFather.Enabled = NricFinCustomValidator.Enabled = isEnabled;
    //}
    #endregion
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateOfDeath.DateIdValue.Length == 0)
            DateOfDeath.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeathCertificateFaEnum.DateOfDeath.ToString(), DateOfDeath.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeath.DateIdValue), DateOfDeath.DateValue);

        if (TagRadioButtonListId.Value.Length == 0)
            TagRadioButtonListId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateFaEnum.Tag.ToString(), TagRadioButtonList.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(TagRadioButtonListId.Value), TagRadioButtonList.SelectedValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateFaEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
        ...
        if (isValidate)
            IdentityNoOfFather.Text = Validation.ReplaceSpecialCharacters(IdentityNoOfFather.Text);
        ...
        if (IdentityNoOfFatherId.Value.Length == 0)
            IdentityNoOfFatherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateFaEnum.IdentityNoOfFather.ToString(), IdentityNoOfFather.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoOfFatherId.Value), IdentityNoOfFather.Text);

        if (NameOfFatherId.Value.Length == 0)
            NameOfFatherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateFaEnum.NameOfFather.ToString(), NameOfFather.Text, docId, false).ToString();
        else
