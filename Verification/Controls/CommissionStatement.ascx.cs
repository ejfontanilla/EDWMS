...
public partial class Verification_Control_CommissionStatement : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "CommissionStatement";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            foreach (MetaData.MetaDataRow metaDataRow in metaDatas.Rows)
            {
                ...
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataCommissionStatementEnum.StartDate.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataCommissionStatementEnum.EndDate.ToString().ToLower()))
                {
                ...
                //else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataCommissionStatementEnum.NameOfCompany.ToString().ToLower()))
                //{
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate, bool IsBlurOrIncomplete)
    {
        ...
        if ((Validation.IsValidDateRangePayExp(FromDate.DateValue, ToDate.DateValue, IsBlurOrIncomplete) &&
            Validation.IsValidDateRangePayExp(FromDate.DateValue, strDateNow, IsBlurOrIncomplete)) || !isValidate)
        {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (FromDate.DateIdValue.Length == 0)
            FromDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataCommissionStatementEnum.StartDate.ToString(), FromDate.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(FromDate.DateIdValue), FromDate.DateValue);

        if (ToDate.DateIdValue.Length == 0)
            ToDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataCommissionStatementEnum.EndDate.ToString(), ToDate.DateValue, docId, false).ToString();
        else
