...
public partial class Verification_Control_GLA : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "GLA";
    ...
    public void ClearData()
    {
        ...
        DateOfIssue.DateValue = IdNo.Text = NameDeceased.Text = string.Empty;
        IDType.SelectedIndex = 0;
        NameDeceasedId.Value = IDTypeId.Value = DateOfIssue.DateIdValue = IdNoId.Value = string.Empty;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            foreach (MetaData.MetaDataRow metaDataRow in metaDatas.Rows)
            {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataPetitionforGLAEnum.NameDeceased.ToString().ToLower()))
                {
                    NameDeceased.Text = metaDataRow.FieldValue;
                    NameDeceasedId.Value = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (isValidate)
            IdNo.Text = Validation.ReplaceSpecialCharacters(IdNo.Text);

        //Added by Edward 2016/06/10 To Take out non letter characters for Name
        if (isValidate)
            NameDeceased.Text = Validation.ReplaceNonLetterCharacters(NameDeceased.Text);
        ...
        if (NameDeceasedId.Value.Length == 0)
            NameDeceasedId.Value = metadataDb.Insert(DocTypeMetaDataPetitionforGLAEnum.NameDeceased.ToString(), NameDeceased.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(NameDeceasedId.Value), NameDeceased.Text);
    }
    #endregion
