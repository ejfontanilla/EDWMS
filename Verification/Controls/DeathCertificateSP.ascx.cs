...
public partial class Verification_Control_DeathCertificateSP : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeathCertificateSP";
    ...
    public void ClearData()
    {
        ...
        NameOfSpouse.Text = IdentityNoOfSpouse.Text = DateOfDeath.DateValue = string.Empty;
        ...
        NameOfSpouseId.Value = IdentityNoOfSpouseId.Value = string.Empty;
        lblDateRangeError.Visible = false;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateSPEnum.DateOfDeath.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateSPEnum.Tag.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateSPEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateSPEnum.IdentityNoOfSpouse.ToString().ToLower()))
                {
                    IdentityNoOfSpouse.Text = metaDataRow.FieldValue;
                    IdentityNoOfSpouseId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateSPEnum.NameOfSpouse.ToString().ToLower()))
                {
    ...
    //{
    //    CommonMetadata.EnableRequiredField = DateOfDeath.EnableRequiredField = RequiredFieldValidatorIdentityNoOfSpouse.Enabled = isEnabled;
    //    RequiredFieldValidatorNameOfSpouse.Enabled = NricFinCustomValidator.Enabled = isEnabled;
    //}
    #endregion
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateOfDeath.DateIdValue.Length == 0)
            DateOfDeath.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeathCertificateSPEnum.DateOfDeath.ToString(), DateOfDeath.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeath.DateIdValue), DateOfDeath.DateValue);

        if (TagRadioButtonListId.Value.Length == 0)
            TagRadioButtonListId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateSPEnum.Tag.ToString(), TagRadioButtonList.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(TagRadioButtonListId.Value), TagRadioButtonList.SelectedValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateSPEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
        ...
        if (isValidate)
            IdentityNoOfSpouse.Text = Validation.ReplaceSpecialCharacters(IdentityNoOfSpouse.Text);
        ...
        if (IdentityNoOfSpouseId.Value.Length == 0)
            IdentityNoOfSpouseId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateSPEnum.IdentityNoOfSpouse.ToString(), IdentityNoOfSpouse.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoOfSpouseId.Value), IdentityNoOfSpouse.Text);

        if (NameOfSpouseId.Value.Length == 0)
            NameOfSpouseId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateSPEnum.NameOfSpouse.ToString(), NameOfSpouse.Text, docId, false).ToString();
        else
