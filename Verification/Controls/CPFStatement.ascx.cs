...
public partial class Verification_Control_CPFStatement : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "CPFStatement";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataCPFStatementEnum.StartDate.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataCPFStatementEnum.EndDate.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (FromDate.DateIdValue.Length == 0)
            FromDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataCPFStatementEnum.StartDate.ToString(), FromDate.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(FromDate.DateIdValue), FromDate.DateValue);

        if (ToDate.DateIdValue.Trim().Length == 0)
            ToDate.DateIdValue = metadataDb.Insert(DocTypeMetaDataCPFStatementEnum.EndDate.ToString(), ToDate.DateValue, docId, false).ToString();
        else
