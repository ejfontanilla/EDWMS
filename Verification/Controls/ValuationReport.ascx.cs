...
public partial class Verification_Control_ValuationReport : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "ValuationReport";
    ...
    public void ClearData()
    {
        ...
        DateOfReport.DateValue = string.Empty;
        DateOfReport.DateIdValue = string.Empty;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataValuationReportEnum.DateOfReport.ToString().ToLower()))
                {
                    DateOfReport.DateValue = Format.GetMetaDataValueInMetaDataDateFormat(metaDataRow.FieldValue);
                    DateOfReport.DateIdValue = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        bool validDate = true;
        lblDateRangeError.Visible = false;
        if (Validation.IsValidDateRange(DateOfReport.DateValue, DateTime.Now.ToString()) || !isValidate)
        {
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate, bool IsBlurOrIncomplete)
    {
        ...
        if (IsBlurOrIncomplete && string.IsNullOrEmpty(DateOfReport.DateValue))
            strDateNow = string.Empty;

        if (Validation.IsValidDateRange(DateOfReport.DateValue, strDateNow, IsBlurOrIncomplete) || !isValidate)
        {
            SaveDataDetails(docId, docRefId, referencePersonalTable);
        }
        else
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfReport.DateIdValue.Length == 0)
            DateOfReport.DateIdValue = metadataDb.Insert(DocTypeMetaDataValuationReportEnum.DateOfReport.ToString(), DateOfReport.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfReport.DateIdValue), DateOfReport.DateValue);
    }
    #endregion
