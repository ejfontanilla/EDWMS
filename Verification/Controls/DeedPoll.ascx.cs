...
public partial class Verification_Control_DeedPoll : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeedPoll";
    ...
    public void ClearData()
    {
        ...
        DateOfDeedPoll.DateValue = string.Empty;
        DateOfDeedPoll.DateIdValue = string.Empty;
    }
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeedPollEnum.DateOfDeedPoll.ToString().ToLower()))
                {
                    DateOfDeedPoll.DateValue = Format.GetMetaDataValueInMetaDataDateFormat(metaDataRow.FieldValue);
                    DateOfDeedPoll.DateIdValue = metaDataRow.Id.ToString();
                }
            }
        }
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        bool validDate = true;
        lblDateRangeError.Visible = false;
        if (Validation.IsValidDateRange(DateOfDeedPoll.DateValue, DateTime.Now.ToString()) || !isValidate)
        {
    ...
    public bool SaveData(int docId, int docRefId, string referencePersonalTable, bool isValidate, bool IsBlurOrIncomplete)
    {
        ...
        if (IsBlurOrIncomplete && string.IsNullOrEmpty(DateOfDeedPoll.DateValue))
            strDateNow = string.Empty;

        if (Validation.IsValidDateRange(DateOfDeedPoll.DateValue, strDateNow, IsBlurOrIncomplete) || !isValidate)
        {
            SaveDataDetails(docId, docRefId, referencePersonalTable);
        }
        else
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfDeedPoll.DateIdValue.Length == 0)
            DateOfDeedPoll.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeedPollEnum.DateOfDeedPoll.ToString(), DateOfDeedPoll.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeedPoll.DateIdValue), DateOfDeedPoll.DateValue);
    }
