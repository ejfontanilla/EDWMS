...
public partial class Verification_Control_DeclarationPrivProp : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeclarationPrivProp";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            foreach (MetaData.MetaDataRow metaDataRow in metaDatas.Rows)
            {
                ...
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeclarationPrivPropEnum.DateOfDeclaration.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfDeclaration.DateIdValue.Length == 0)
            DateOfDeclaration.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeclarationPrivPropEnum.DateOfDeclaration.ToString(), DateOfDeclaration.DateValue, docId, false).ToString();
        else
