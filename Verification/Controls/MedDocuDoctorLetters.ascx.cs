...
public partial class Verification_Controls_MedDocuDoctorLetters : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "MedDocuDoctorLetters";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataMedDocuDoctorLettersEnum.DateOfDocument.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfDocument.DateIdValue.Length == 0)
            DateOfDocument.DateIdValue = metadataDb.Insert(DocTypeMetaDataMedDocuDoctorLettersEnum.DateOfDocument.ToString(), DateOfDocument.DateValue, docId, false).ToString();
        else
