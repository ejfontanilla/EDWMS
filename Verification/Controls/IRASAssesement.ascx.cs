...
public partial class Verification_Control_IRASAssesement : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "IRASAssesement";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIRASAssesementEnum.YearOfAssessment.ToString().ToLower()))
                {
                    YearOfAssessment.Text = metaDataRow.FieldValue;
                    YearOfAssessmentId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIRASAssesementEnum.DateOfFiling.ToString().ToLower()))
                {
                ...
                //else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataIRASAssesementEnum.TypeOfIncome.ToString().ToLower()))
                //{
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        { 
            YearOfAssessmentId.Value = metadataDb.Insert(DocTypeMetaDataIRASAssesementEnum.YearOfAssessment.ToString(), YearOfAssessment.Text, docId, false).ToString();
            FromDateId.Value = metadataDb.Insert(DocTypeMetaDataPAYSLIPEnum.StartDate.ToString(), string.Format("01-01-{0}", YearOfAssessment.Text), docId, false).ToString();
            ToDateId.Value = metadataDb.Insert(DocTypeMetaDataPAYSLIPEnum.EndDate.ToString(), string.Format("31-12-{0}", YearOfAssessment.Text), docId, false).ToString();
        }
        else
        {
            metadataDb.Update(int.Parse(YearOfAssessmentId.Value), YearOfAssessment.Text);
            metadataDb.Update(int.Parse(FromDateId.Value), string.Format("01-01-{0}", YearOfAssessment.Text));
            metadataDb.Update(int.Parse(ToDateId.Value), string.Format("31-12-{0}", YearOfAssessment.Text));
        }
            
        if (DateOfFiling.DateIdValue.Length == 0)
            DateOfFiling.DateIdValue = metadataDb.Insert(DocTypeMetaDataIRASAssesementEnum.DateOfFiling.ToString(), DateOfFiling.DateValue, docId, false).ToString();
        else
