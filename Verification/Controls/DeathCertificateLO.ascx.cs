...
public partial class Verification_Controls_DeathCertificateLO : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "DeathCertificateLO";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateLOEnum.DateOfDeath.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateLOEnum.Tag.ToString().ToLower()))
                {
                ...
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateLOEnum.IDType.ToString().ToLower()))
                {
                    if (metaDataRow.FieldValue.Length > 0) IDType.SelectedValue = metaDataRow.FieldValue;
                    IDTypeId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateLOEnum.IdentityNoOfLateOwner.ToString().ToLower()))
                {
                    IdentityNoOfMother.Text = metaDataRow.FieldValue;
                    IdentityNoOfMotherId.Value = metaDataRow.Id.ToString();
                }
                else if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataDeathCertificateLOEnum.NameOfLateOwner.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable, bool isValidate)
    {
        ...
        if (DateOfDeath.DateIdValue.Length == 0)
            DateOfDeath.DateIdValue = metadataDb.Insert(DocTypeMetaDataDeathCertificateLOEnum.DateOfDeath.ToString(), DateOfDeath.DateValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(DateOfDeath.DateIdValue), DateOfDeath.DateValue);

        if (TagRadioButtonListId.Value.Length == 0)
            TagRadioButtonListId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateLOEnum.Tag.ToString(), TagRadioButtonList.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(TagRadioButtonListId.Value), TagRadioButtonList.SelectedValue);

        if (IDTypeId.Value.Length == 0)
            IDTypeId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateLOEnum.IDType.ToString(), IDType.SelectedValue, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IDTypeId.Value), IDType.SelectedValue);

        if (IdentityNoOfMotherId.Value.Length == 0)
            IdentityNoOfMotherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateLOEnum.IdentityNoOfLateOwner.ToString(), IdentityNoOfMother.Text, docId, false).ToString();
        else
            metadataDb.Update(int.Parse(IdentityNoOfMotherId.Value), IdentityNoOfMother.Text);

        if (NameOfMotherId.Value.Length == 0)
            NameOfMotherId.Value = metadataDb.Insert(DocTypeMetaDataDeathCertificateLOEnum.NameOfLateOwner.ToString(), NameOfMother.Text, docId, false).ToString();
        else
