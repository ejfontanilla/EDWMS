...
public partial class Verification_Control_LoanStatementSold : System.Web.UI.UserControl
{
    private String _CurrentDocType;
    protected string DocType = "LoanStatementSold";
    ...
    public void LoadData(int docId, int docRefId, string referencePersonalTable, Boolean isUnderHouseholdStructure)
    {
        ...
        if (CurrentDocType.ToLower().Equals(DocType.ToLower()))
        {
            ...
            {
                if (metaDataRow.FieldName.ToLower().Trim().Equals(DocTypeMetaDataLoanStatementSoldEnum.DateOfStatement.ToString().ToLower()))
                {
    ...
    private void SaveDataDetails(int docId, int docRefId, string referencePersonalTable)
    {
        ...
        if (DateOfStatement.DateIdValue.Length == 0)
            DateOfStatement.DateIdValue = metadataDb.Insert(DocTypeMetaDataLoanStatementSoldEnum.DateOfStatement.ToString(), DateOfStatement.DateValue, docId, false).ToString();
        else
